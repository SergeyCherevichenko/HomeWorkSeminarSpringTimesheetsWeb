...
public class TimesheetController {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update timesheet", description = "Обновить запись по ее идентификатору",
            responses = {
                    @ApiResponse(description = "Успешный ответ", responseCode = "200", content = @Content(schema = @Schema(implementation = Timesheet.class))),
                    @ApiResponse(description = "Запись не найдена", responseCode = "404", content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
                    @ApiResponse(description = "Внутренняя ошибка", responseCode = "500", content = @Content(schema = @Schema(implementation = Void.class)))
            })
    public ResponseEntity<Timesheet> updateTimesheet(@PathVariable Long id, @RequestBody Timesheet timesheetDetails) {
        try {
            Timesheet updatedTimesheet = service.updateTimesheet(id, timesheetDetails);
            return ResponseEntity.ok(updatedTimesheet); // 200 OK
